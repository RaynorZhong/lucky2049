...
浏览器页面([`/randomness.html`](https://lucky2049.com/randomness.html))渲染一个**实时近窗**——约最近
18 枚硬币 + 6 颗骰子——靠在访客浏览器里每 ~60 秒轮询链尖保持最新:来一个新块就翻入一枚硬币,凑齐一个 6
块窗口就掷出一颗骰子。它是**效果演示**,所以直接读**最新**区块(不等确认);链尖最近几块可能被重组,下次
轮询会自动校正。不发布、不存储——每个结果都是即时重算、人人可验证。
